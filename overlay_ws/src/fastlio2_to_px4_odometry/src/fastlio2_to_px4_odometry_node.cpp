...
class FastLio2ToPx4OdometryNode : public rclcpp::Node
{
public:
  FastLio2ToPx4OdometryNode()
  : Node("fastlio2_to_px4_odometry")
  {
    const std::string input_topic = this->declare_parameter<std::string>(
      "input_topic", "/drone_1/aft_mapped_to_init_level");
    const std::string output_topic = this->declare_parameter<std::string>(
      "output_topic", "/drone_1/fmu/in/vehicle_visual_odometry");
    ...
    subscription_ = this->create_subscription<nav_msgs::msg::Odometry>(
      input_topic,
      20,
      std::bind(&FastLio2ToPx4OdometryNode::handle_odometry, this, std::placeholders::_1));
  }
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<fastlio2_to_px4_odometry::FastLio2ToPx4OdometryNode>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
