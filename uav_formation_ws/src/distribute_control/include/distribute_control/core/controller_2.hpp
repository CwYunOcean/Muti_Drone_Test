...
#include <Eigen/Dense>
#include <memory>
#include "distribute_control/curves/curve_base_2.hpp"

namespace distribute_control
{

    // 控制参数结构体 (方便一次性传参)
    struct ControlParams_2
    {
    ...
    // 控制器输出结构体
    struct ControlOutput_2
    {
        Eigen::Vector3d velocity;      // 速度指令
        double w1_updated;             // 更新后的 w1
        double w2_updated;             // 更新后的 w2
        Eigen::Vector3d target_pos;    // 调试用：目标位置
        Eigen::Vector3d current_error; // 调试用：误差
    };

    class DistributeController_2
    {
    public:
        DistributeController_2();

        // 设置使用的曲线 (策略模式)
        void set_curve(std::shared_ptr<CurveBase_2> curve);
        // 更新参数
        void update_params(const ControlParams_2 &params);
         ...
         * @param dt 时间步长 (0.01s)
         * @return ControlOutput_2 包含速度指令和新的 w
         */
        ControlOutput_2 compute_control(
            ...
            double delta1_next, double delta2_next, double target_z,
            double dt);

    private:
        std::shared_ptr<CurveBase_2> curve_; // 当前使用的曲线
        ControlParams_2 params_;
    };
