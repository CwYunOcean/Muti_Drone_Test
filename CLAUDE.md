...
| `slam_ws/src/` | Vendored FAST-LIO, FAST-LIVO, Livox, and calibration baseline | Shared source; changes require review and testing |
...
`manifests/` holds pinned external upstream `.repos` files. `nav_ws/src/ego-swarm-ros2` and `slam_ws/src` are already present after a root clone; their provenance and license obligations are recorded in `nav_ws/UPSTREAM.md` and `slam_ws/UPSTREAM.md`. Do not commit build artifacts, rosbags, map dumps, sensor recordings, runtime logs, or calibration output. Keep machine-specific edits (e.g., lidar IPs) local unless they belong in a tracked runbook or config template.
...
- `main` holds docs, manifests, vendored navigation and SLAM sources, and reviewed overlay code; use project-local `.worktrees/<branch>` for feature branches
...
- Changes below `slam_ws/src` are shared source changes. Keep upstream licenses, state material deviations in the commit, rebuild affected SLAM and overlay packages, and keep generated logs, PCD files, and calibration output local.
